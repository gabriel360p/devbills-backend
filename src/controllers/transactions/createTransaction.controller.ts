...
export const storeTransaction = async(request:FastifyRequest, reply:FastifyReply):Promise<void> =>{
    ...
    if(!result.success) return reply.status(400).send({message:"Erro de validação", error:result.error})
    ...
    try {
        ...
        if(!result.success) return reply.status(400).send({message:"Categoria inexistente"})
